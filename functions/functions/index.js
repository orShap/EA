...
var getRangeReturns = async(function(start, end) {
	...
	if (start < end) {
		...
		if (symbolsToBuy.exists() && positionReturns.exists()) {
			...
			while (curr <= end) {
				//await (getReportedEarnings(curr));
				let arrInfo = [];
...
var getReportedEarnings = async(function(date) {
		...
		if (date) today = utils.clearFormatedTZDate(moment.tz(date, "America/New_York")).substring(0,10);
...
//getRangeReturns("2018-05-01", "2018-08-05");
		...
		if (cPositionsInfo)
			Object.keys(eaSymbolsToBuy[currD]).forEach(i => {
				...
				if (position && share) {
					...
					var datesKeyes = Object.keys(share).sort();
					let i = 0;
					datesKeyes.forEach(di => { 
						if (share[di].date == wdate) 
							index = i;
						i++;
					})
					if (index != null) {
						let baseDate = datesKeyes[index - 4];
						if (!baseDate)
							return null;

						for (let x = index - 3; x <= index && x < datesKeyes.length; x++) {
							let nextDate = datesKeyes[x];
							volumes += (share[nextDate].volume && share[baseDate].volume ? 
													share[nextDate].volume / share[baseDate].volume : "") + ',';
						}
						for (let x = index - 3; x <= index && x < datesKeyes.length; x++) {
							let prevDate = datesKeyes[x - 1];
							let currDate = datesKeyes[x];
							changes +=
							  (share[currDate].close / share[currDate].open) + ',' + 
								(share[currDate].open / share[prevDate].close) + ',' + 
								(share[currDate].close / share[prevDate].close) + ',';
							}
					}
					else 
						return null;
